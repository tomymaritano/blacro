...
async function updatePrivatelimoImages() {
  console.log('🚀 Actualizando imágenes de Private Limo en Cloudinary...\n');
  
  const privatelimoDir = path.join(__dirname, 'public/privatelimo');
  const files = fs.readdirSync(privatelimoDir).filter(f => 
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const filePath = path.join(privatelimoDir, file);
    const nameWithoutExt = path.basename(file, path.extname(file));
    const publicId = `blacro/privatelimo/${nameWithoutExt}`;
...
updatePrivatelimoImages().catch(console.error);