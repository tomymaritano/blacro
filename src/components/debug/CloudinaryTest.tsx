  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="bg-blue-50 p-4 rounded">
        ...
        <p>Cloud Name: {cloudName || 'NOT SET'}</p>
        <p>Node ENV: {process.env.NODE_ENV}</p>
        {!cloudName && (
          <div className="mt-2 p-2 bg-yellow-100 border border-yellow-400 rounded">
            <p className="text-yellow-800">⚠️ Cloudinary not configured. CldImage tests will be skipped.</p>
          </div>
        )}
      </div>
      ...
      {cloudName ? (
        ...
        <div className="border p-4 rounded bg-gray-50">
          <h3 className="font-semibold mb-4">1. Basic CldImage Test (SKIPPED)</h3>
          <p className="text-gray-600">CldImage test skipped - Cloudinary not configured</p>
        </div>
      )}
