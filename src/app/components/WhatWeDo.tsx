...
export default function WhatWeDoSection() {
  ...
  return (
    <section className="w-full px-4 sm:px-8 py-12 text-black flex flex-col items-center space-y-8">
      <span className="text-xs uppercase border border-black rounded-full px-4 py-1 tracking-wide font-grotesk">
        What we do
      </span>

      <p className="text-center max-w-3xl text-base sm:text-lg leading-relaxed tracking-wide">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 w-full max-w-6xl">
        {columns.map((col, index) => (
          ...
          >
            <h3 className="font-bold text-base sm:text-lg mb-2">{col.title}</h3>
            <div className="flex flex-col space-y-2">
              {col.items.map((item, i) => (
                <div
                  key={i}
                  className="py-2 border-b border-black/20 text-sm sm:text-base uppercase cursor-pointer transition-all hover:pl-2"
                >
                  {item}
                </div>
              ))}
